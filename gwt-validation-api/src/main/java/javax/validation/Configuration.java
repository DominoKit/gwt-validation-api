...
/**
 * Receives configuration information, selects the appropriate Jakarta Bean Validation provider and
 * builds the appropriate {@link ValidatorFactory}.
 *
 * <p>Usage:
 *
 * <pre>
 * // provided by one of the Validation bootstrap methods
 * Configuration&lt;?&gt; configuration = ValidatorFactory =
 *     configuration.messageInterpolator(new CustomMessageInterpolator()).buildValidatorFactory();
 * </pre>
 *
 * <p>By default, the configuration information is retrieved from {@code META-INF/validation.xml}.
 * It is possible to override the configuration retrieved from the XML file by using one or more of
 * the {@code Configuration} methods.
 *
 * <p>The {@link ValidationProviderResolver} is specified at configuration time (see {@link
 * ValidationProvider}). If none is explicitly requested, the default {@code
 * ValidationProviderResolver} is used.
 *
 * <p>The provider is selected in the following way:
 *
 * <ul>
 *   <li>if a specific provider is requested programmatically using {@link
 *       Validation#byProvider(Class)}, find the first provider implementing the provider class
 *       requested and use it
 *   <li>if a specific provider is requested in {@code META-INF/validation.xml}, find the first
 *       provider implementing the provider class requested and use it
 *   <li>otherwise, use the first provider returned by the {@code ValidationProviderResolver}
 * </ul>
 *
 * <p>Implementations are not meant to be thread-safe.
 *
...
public interface Configuration<T extends Configuration<T>> {
   ...
   *
   * <p>This method is typically useful for containers that parse {@code META-INF/validation.xml}
   ...
   * interpolator.
   *
   * <p>If {@code null} is passed, the default message interpolator is used (defined in XML or the
   ...
   * resolver.
   *
   * <p>If {@code null} is passed, the default traversable resolver is used (defined in XML or the
   * specification default).
   *
   ...
   * factory.
   *
   * <p>If {@code null} is passed, the default constraint validator factory is used (defined in XML
   ...
   *
   * <p>If {@code null} is passed, the default parameter name provider is used (defined in XML or
   ...
   *
   * <p>If {@code null} is passed, the default clock provider is used (defined in XML or the
   * specification default).
   *
   ...
   *
   * <p>The stream should be closed by the client API after the {@link ValidatorFactory} has been
   * built. The Jakarta Bean Validation provider must not close the stream.
   *
   ...
   *     supporting the mark/reset contract by the Jakarta Bean Validation provider
   * @return {@code this} following the chaining method pattern
   ...
   * it.
   *
   * <p>Note: Using this non type-safe method is generally not recommended.
   *
   * <p>It is more appropriate to use, if available, the type-safe equivalent provided by a specific
   ...
   *
   * <p>If a property with a given name is defined both via this method and in the XML
   * configuration, the value set programmatically has priority.
   *
   * <p>If {@code null} is passed as a value, the value defined in XML is used. If no value is
   * defined in XML, the property is considered unset.
   *
   ...
   *
   * <ul>
   *   <li>use the {@code ValidationMessages} resource bundle to load keys
   *   <li>use {@code Locale.getDefault()}
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>uses the public no-arg constructor of the {@link ConstraintValidator}
   * </ul>
   *
   ...
   *
   * <ul>
   *   <li>returns the actual parameter names as provided in the validated executable’s definition,
   *       if the class file of the executable contains parameter name information
   *   <li>otherwise returns names in the form {@code arg&lt;PARAMETER_INDEX&gt;}, where {@code
   *       PARAMETER_INDEX} starts at 0 for the first parameter, e.g. {@code arg0}, {@code arg1}
   *       etc.
   * </ul>
   *
  ...
  /**
   * Returns an implementation of the {@link ClockProvider} interface following the default {@code
   * ClockProvider} defined in the specification:
   *
   * <ul>
   *   <li>returns a clock representing the current system time and default time zone.
   * </ul>
   *
   ...
   *
   * @return returns an instance of {@link BootstrapConfiguration}; this method never returns {@code
   *     null}; if there is no {@code META-INF/validation.xml} the different getters of the returned
   *     instance will return {@code null} respectively an empty set or map
   * @since 1.1
   */
